...
- **`after_goal` now actually forwards `GOAL_*` env (W1019)** — the 1.2.0 changelog claimed `GOAL_ID` / `GOAL_IDENTIFIER` / `GOAL_TITLE` / `GOAL_DESCRIPTION` were forwarded into the `## after_goal` child process via `buildHookEnv`, but no code parsed the server's `hook.env` for the after_goal entry — `buildHookEnv` only merged the six `TASK_*` keys plus `HOOK_NAME`, so any `## after_goal` script referencing `$GOAL_IDENTIFIER` saw an empty value. `extensions/hook-bridge/after-goal-detector.ts` gains `extractGoalEnvFromResult(content, details)`, which finds the `after_goal` entry in the response `hooks` array and returns its `env` block — `GOAL_*` plus `BOARD_*` / `COLUMN_*` / `AGENT_NAME` — sourced verbatim from `hook.env` (the single source of truth per `stride/skills/stride-workflow/hook-execution.md`). Canonical keys the server omits default to the empty string (defined-but-empty, so `set -u` scripts do not abort); prototype-pollution keys (`__proto__` / `constructor` / `prototype`) and non-string values are dropped defensively, and the Stride API bearer token is never injected (it is not part of `hook.env`). `buildHookEnv` / `runHook` gain an optional `extraEnv` parameter so the forwarded goal env is spread on top of the cache before `HOOK_NAME`.
- **`after_goal` env-cache deletion ordering on the mark_reviewed route (W1019)** — on `/mark_reviewed` (the `after_review` hook), the env cache was deleted *before* the after_goal block ran, so after_goal executed with no cached env at all. The post-hook sequence moved into a new `extensions/hook-bridge/after-goal-runner.ts` (`runAfterGoalAndCleanup`), which runs `## after_goal` first and only then deletes the cache — mirroring the canonical `stride/hooks/stride-hook.sh` ordering (lines 603-630). The runner is a pure, import-free orchestrator with every collaborator injected, so the ordering and the goal-env forwarding are unit-tested under plain `node --test`.

### Tests

- **`extensions/hook-bridge/index.test.ts`** — 13 new `node:test` cases (77 total in the suite): 7 for `extractGoalEnvFromResult` (verbatim forwarding from wrapped + raw shapes, `details.output` preference, empty-string defaults for omitted/absent env, the no-API-token guarantee, and the prototype-pollution / non-string drops) and 6 for `runAfterGoalAndCleanup` (after_goal-before-cleanup ordering on the after_review route, no-cleanup on before_review, cleanup-still-runs when no after_goal, skip-on-primary-failure, no-op on non-review hooks, and the null-result no-emit path). The runner tests inject the real detector functions so they exercise `responseHasAfterGoal` / `extractGoalEnvFromResult` end-to-end.

### Source

G166 / W1019. Corrects two HIGH-severity after_goal hook-bridge bugs and the documentation that asserted the unimplemented `GOAL_*` forwarding as working.
...
- **`GOAL_*` env vars** — `GOAL_ID`, `GOAL_IDENTIFIER`, `GOAL_TITLE`, `GOAL_DESCRIPTION` intended to be forwarded into the `## after_goal` child process environment, sourced verbatim from the server-supplied `hook.env`. `BOARD_*`, `COLUMN_*`, `AGENT_NAME`, and `HOOK_NAME` remain present across all five hooks. **Correction (W1019, see [Unreleased]):** this forwarding was documented but never implemented in 1.2.0 — `buildHookEnv` merged only the six `TASK_*` keys plus `HOOK_NAME` and nothing parsed `hook.env` for the after_goal entry, so `GOAL_*` were empty at runtime. The Unreleased fix actually implements it.
