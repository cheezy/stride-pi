...
`task-enricher` runs **before claiming** a sparse task (enrich → PATCH → re-fetch → claim); the others run after claim per the `stride-subagent-workflow` decision matrix. The `task-reviewer` emits a `schema_version` 1.7 structured review block (with `security_considerations` and `project_checks`; the `project_checks` per-entry `status` enum is `met`/`not_met`/`not_applicable` with the full checklist emitted).
