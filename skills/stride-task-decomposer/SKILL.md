...
Use the codebase to inform your decomposition. Read `AGENTS.md` / `CLAUDE.md` for project conventions and explore `lib/` and `test/` for existing implementations related to the goal.
...
2. **Search the codebase** for existing implementations related to the goal:
   ```bash
   grep -rnE "<goal keyword>|<synonym>" lib/ test/
...
Produce the JSON payload above; the caller POSTs it to `/api/tasks/batch`. See `stride-creating-goals/SKILL.md` for the full batch format and `stride-creating-tasks/SKILL.md` for the per-task field requirements (those skills own the field schemas — this skill covers the *decomposition* logic). Do not interact with the Stride API from this skill — you produce the payload; the caller posts it.
...
- **Do NOT create dependencies across goals in batch requests** — create sequentially instead. Flag any dependency on work **outside** this goal using existing task identifiers (e.g. `["W47"]`) rather than indices.
- **Do NOT produce minimal nested tasks** — each task needs full specification per `stride-creating-tasks`, including `security_considerations` (array of strings, or an explicit "None — …" reason)
