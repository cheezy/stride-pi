...
| `dispatch_count` | integer | Optional, when `dispatched=true` | How many times this step's subagent was invoked (W2130) — an inline run counts too, since Step 5 reports both paths as `dispatched: true` — and the cost unit, because an invocation that crashed still spent its tokens. It counts dispatches, never rounds, so it is never filled from the round count; what the number can and cannot be read to mean is set out below, and leaving the key off is always valid |
...
### What `dispatch_count` Can and Cannot Tell You

**Canon-governed — entry `dispatch-count-telemetry` in `stride/docs/port-canon.md`.** That entry registers the key, its counts-dispatches-not-rounds rule, and a set of six limits every port carries. Three of them — (2), (3) and (5) — are restated below in this port's own terms, because the mechanisms stride's wording names do not all exist here; **those restatements preserve each limit's substance and so owe no version bump.** What would owe one is changing what a limit *claims*, in either direction — and that owes it in two places before the next release: the canon entry, and the anchor above. As with the cap at Step 5, the checker tests the anchor, never the sentence.

**Writing it.** Put `dispatch_count` on every `dispatched: true` entry whose count you know — **a `1` included**. Count invocations of either kind, one re-run after a crash among them, and never fill the field from the round count, which by the Step 5 definition deliberately excludes a crash. Omitting the key stays valid, so an older version of this port completes exactly as it does today; but an omission you chose cannot be told apart from one that could not be avoided, which is the ambiguity limit (4) describes.

**These limits are stated rather than papered over, because a figure trusted past its accuracy is worse than no figure.** stride keeps them in a companion file; every skill here is a single `SKILL.md`, so they live here. Three of them — (2), (3) and (5) — say something different on this port than they do in stride, and the difference is stated rather than smoothed over.

**(1) Wall-clock is not token cost, and the two disagree about which task was dearer.** Across the eight real dispatches measured when the key was introduced (W2130), tokens per second varied about **2.1×** by dispatch kind — reviewer rounds around 300–375 tok/s, specialist security dispatches around 455–625 tok/s. The figure tracks a task's review *mix*, not its size. On the two real records available then, the pair ranked one task 20.3% more expensive when its token cost was in fact 1.4% *cheaper*: the order inverted, and the recorded gap was some fifteen times the real one. **Use the pair to see that a review phase ran long; never to conclude it was the costlier of two.**

**(2) The two keys measure the same population here — which removes stride's hazard and leaves a smaller one.** In stride the `reviewer` entry's `duration_ms` is an aggregate: a deep security review, an exploratory session and a hardening step all fold their wall-clock into it, so dividing by the count mixes populations. **None of those three steps exists on this port** — there is no dispatched security specialist, no exploratory-testing step and no hardening step — so nothing folds in, and `duration_ms / dispatch_count` really is the mean wall-clock of one reviewer invocation. **It is still not a per-round figure.** By the Step 5 definition a round is an invocation whose response yielded a parsed block, and one that crashed or returned no parsable block consumes no round while still costing a dispatch. So the divisor counts dispatches the numerator paid for but the cap never charged, and nothing in the record says how many. **Read it as a mean per dispatch, never as a mean per round.**

**(3) Two different keys are spelled `dispatched`, and confusing them is the mistake this port invites.** `reviewer_result.dispatched` and the `reviewer` entry's `workflow_steps[].dispatched` are **not** the same flag and do not move together. On this runtime the reviewer commonly runs **inline** rather than as a dispatched subprocess, and on that path `reviewer_result` takes the self-reported skip form — but the **telemetry entry is still `dispatched: true`**, because the work was genuinely performed rather than skipped, and Step 5 says so outright: both invocation paths report `dispatched: true`. The worked payloads in this skill and in `stride-completing-tasks` show exactly that pairing. **So `dispatch_count` is recordable on this port's ordinary path, not only when a subagent was spawned** — count the reviewer's invocations, inline ones included. And when you read a record back, a `reviewer_result.dispatched: false` sitting beside a `reviewer` entry carrying `dispatched: true` and a `dispatch_count` is **not** a contradiction and must not be repaired into one. A `dispatched: false` reviewer entry then has **two** readings, and they are not equally honest. Under `decision_matrix_skip` no review ran at all, and the missing duration and count cost nothing — on this port, unlike stride, no security specialist or exploratory session could have been running in the background either. Under `ran_inline` the review **did** run and the entry records none of what it cost, which is the fleet canon's own limit (3) and is live here. **For the reviewer, Step 5 settles which to write: an inline invocation is `dispatched: true`, with a duration and, where you know it, a count.** So `ran_inline` on a reviewer entry is a record that loses real cost rather than a second correct way to spell one — and wherever you meet one, **absence of a cost figure is not evidence of absent cost.**

**(4) An omitted `dispatch_count` is ambiguous, and a reader must not impute a value.** It may mean a version predating the key, a matrix-skipped review per (3), or an oversight. Imputing `1` rather than a higher value moved a sample mean by over 100% on that same W2130 sample. **When aggregating, report the covered subset and its size instead of filling absences.** The ambiguity is widest during rollout, which is exactly when aggregating is most tempting.

**(5) It cannot separate a crashed re-dispatch from a genuine extra round, and nothing on this port can recover the difference.** A `2` may be one round plus one crash, or two rounds — deliberate, since the count measures cost and a crashed dispatch really spent its tokens. The consequence needs saying plainly: **a fully compliant `3`, two rounds plus a crash, reads like a breach of the two-round cap to anyone who knows that cap. It is not one.** stride can settle such a question afterwards from a persisted round-count artifact. **This port writes none** — the count lives only in your memory, as the cap's own disclosure says, and a structured round field is forbidden rather than merely missing. So the distinction is gone the moment the session ends. Two things follow: **never read a cap breach out of `dispatch_count` alone**, and when a count exceeds the rounds you actually ran, **say in `completion_notes` what it meant** — that sentence is the only record there will ever be.

**(6) Nothing validates the value on the way in — not this port, not the server.** This port *does* have an executable self-check, so the absence is not for want of a mechanism: the Python block under "Extracting the structured review block" runs over `structured` and the task's own criteria, and touches `workflow_steps` nowhere. Nothing else here does either — the hook bridge matches the claim, complete and mark_reviewed routes and never inspects a completion body's telemetry. Server-side, `valid_step?/1` checks `name`, `dispatched`, `duration_ms` and `reason`, with the `reason_code` enum validated separately, and does not check this key. Unlike `reason_code`, which is refused when unrecognised, `dispatch_count` is accepted whatever its type, stored in the array and returned verbatim. Nothing reads it yet, so today's cost is a corrupt record rather than anything worse. **Whatever reads it first — a renderer, a dashboard, an aggregation — must guard for a non-integer itself, unless the validator first gains the optional-but-validated shape `reason_code` already has.**

**Why not simply record tokens.** The obvious answer to (1) is a token count, the one figure observed to rank correctly. The task that introduced these keys declined it — record what is actually measurable — and that stands. Be exact about the reason, though, because the usual one is wrong: some runtimes **do** measure a per-dispatch token cost, so recording it would not be inventing it. The unsettled question is **portability**, whether every runtime this schema serves can produce one. Until that is answered, this pair is what exists, read within the limits above.
